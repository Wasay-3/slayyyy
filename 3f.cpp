...
int bricks[5][10];
char difficulty;
double tiltAngle; // New variable to store the tilt angle of the slider
...
void initialize() {
    ...
    dy2 = -2;
    score = 0;
    tiltAngle = 0.0; // Initialize tilt angle to 0
...
// Setting Background color w.r.t Difficulty level
void drawbackground() {
    int bgColor;
...
void draw() {
    ...
    // Draw background color regarding the difficulty level
    setfillstyle(SOLID_FILL, BLACK);
    bar(paddle_x, paddle_y, paddle_x + 100, paddle_y + 10);

    // Draw the paddle as a solid filled rectangle using the specified coordinates and tilt angle
    setcolor(13);
    setfillstyle(HATCH_FILL, 13);
    int rotatedPaddleX = paddle_x + 50 + int(30 * sin(tiltAngle));
    int rotatedPaddleY = paddle_y + int(30 * cos(tiltAngle));
    bar(rotatedPaddleX - 50, rotatedPaddleY - 5, rotatedPaddleX + 50, rotatedPaddleY + 5);
...
void input() {
    // Move the paddle left and right by arrow keys
    ...
    // Tilt the paddle left and right
    if (GetAsyncKeyState('A') & 0x8000) {
        paddle_x -= 20;
        paddle_y -= 10; // Adjust the tilt angle as needed
    }
    ...
    if (paddle_y >= getmaxy() - 10) {
        paddle_y = getmaxy() - 10;
    }

    // Update the position of the ball based on the position of the paddle
    ball_x = paddle_x + 50;
    ball_y = paddle_y - 10;
}
